...
parameters = {
    ...
    # Paths
    "data_path" :
        Path("D:\local_Anderau\data_old"),
    # "data_path" :
    #     Path(r"\\scopem-idadata.ethz.ch\BDehapiot\remote_Anderau\data_new"),
    "model_name" :
...
#%% Class(Main) : process() ---------------------------------------------------
    ...
    def process(self):
        ...
        def get_synaptic_plane_dist(C3):
            z_max = np.argmax(rescale(C3, (1, 0.05, 0.05)), axis=0)
            z_max = resize(z_max, C3[0].shape, order=0, preserve_range=True)
            sdist = np.arange(C3.shape[0])[:, np.newaxis, np.newaxis]
            sdist = np.broadcast_to(sdist, C3.shape)
            sdist = sdist - z_max
            return sdist
        
        def subtract_background(Cx, all_lbl, kernel_size=9, coeff=1.0):       
            msk = all_lbl == 0
            ...
            bsub = Cx.copy().astype(float) - (bgrd * coeff)
        ...
        def _process(path):
            ...
            if lbl_path.exists() and self.procedure["process"] == 1:
                ...
                sdist = get_synaptic_plane_dist(C3)
                ...
                if bsub:
                    C1 = subtract_background(
                        C1, all_lbl, kernel_size=9, 
                        coeff=self.parameters["bsub_coeff"]
                        )
                    C2 = subtract_background(
                        C2, all_lbl, kernel_size=9, 
                        coeff=self.parameters["bsub_coeff"]
                        )       
...
#%% Class(Main) : analyse() ---------------------------------------------------
    ...
    def analyse(self):
        ...
        tps = []
        for path in self.paths:
            tps.append(int(path.stem.split("_")[1][:2]))
        timepoints = []
        for tp in np.unique(tps):
            timepoints.append(f"{tp:02d}min")
